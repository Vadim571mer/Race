...
public class RCCP_WelcomeWindow : EditorWindow
{
    ...
    public static void WelcomePageContent()
    {
        ...
        GUILayout.Label(
            "Thank you for purchasing and using Realistic Car Controller Pro. Please read the documentations before use. Also check out the online documentations for updated info. Have fun :)");
        ...
        EditorGUILayout.HelpBox(
            "Demo assets are not included. If you want to use demo assets (vehicles, scenes, examples, etc...), you must import demo assets to your project. You can import them from ''Addons'' tab.",
    ...
    public static void UpdatePageContent()
    {
        ...
        EditorGUILayout.BeginHorizontal(GUI.skin.box);
        GUILayout.Label(
            "<b>2</b>- If you have own assets such as prefabs, audioclips, models, scripts in Realistic Car Controller Pro folder, keep your own asset outside from Realistic Car Controller Pro folder.");
        EditorGUILayout.EndHorizontal();
        GUILayout.Space(6);

        EditorGUILayout.BeginHorizontal(GUI.skin.box);
        GUILayout.Label(
            "<b>3</b>- Delete Realistic Car Controller Pro folder, and import latest version to your project.");
        EditorGUILayout.EndHorizontal();
        GUILayout.Space(6);
    ...
    public static void ShadersContent()
    {
        ...
        EditorGUILayout.BeginHorizontal(GUI.skin.box);
        GUILayout.Label(
            "RCCP comes with builtin render pipeline shaders. If your project is running on URP or HDRP, you'll need to convert demo materials of the RCCP and import URP shaders. There are four simple steps to do this process. \n\n1. Importing URP shaders\n\n2. Selecting all demo materials of the RCCP in the project, and converting them through Edit --> Rendering --> Convert menu." +
        ...
        EditorGUILayout.BeginHorizontal(GUI.skin.box);
        GUILayout.Label(
            "All these steps can be done through the Tools --> BCG --> RCCP --> URP --> To URP / To Builtin menu, or welcome window.");
        EditorGUILayout.EndHorizontal();
        GUILayout.Space(6);
    ...
    public static void DemosPageContent()
    {
        ...
        if (GUILayout.Button("Import Demo Content"))
            decisionToImportDemo = EditorUtility.DisplayDialog("Import Demo Content",
                "Do you want to import demo assets to your project? Be adviced, this will increase your build size even if you don't use them. You can always remove the demo assets from your project by using 'Delete Demo Content From Project' button in welcome window.",
                "Yes, import demo assets", "No");

        if (decisionToImportDemo)
            AssetDatabase.ImportPackage(
                RCCP_AddonPackages.Instance.GetAssetPath(RCCP_AddonPackages.Instance.demoPackage), true);
...
#if !RCCP_DEMO
        EditorGUILayout.HelpBox("Demo assets are not included. You can import them from ''Addons'' tab.",
            MessageType.Info, true);
        GUI.enabled = false;
#endif

        EditorGUILayout.Separator();
        EditorGUILayout.HelpBox("All scenes must be in your Build Settings to run AIO demo.", MessageType.Warning,
            true);
        EditorGUILayout.Separator();
        ...
        EditorGUILayout.HelpBox(
            "If you want to add enter exit scenes, you have to import latest BCG Shared Assets to your project first.",
            MessageType.Warning);

        if (GUILayout.Button("Download and import BCG Shared Assets"))
            AssetDatabase.ImportPackage(
                RCCP_AddonPackages.Instance.GetAssetPath(RCCP_AddonPackages.Instance.BCGSharedAssets), true);
        ...
        if (!photonAndRCCInstalled)
        {
            ...
            {
                EditorGUILayout.HelpBox(
                    "If you want to add Photon PUN2 scenes, you have to import latest Photon PUN2 to your project first.",
                    MessageType.Warning);
                EditorGUILayout.HelpBox("You have to import latest Photon PUN2 to your project first.",
                    MessageType.Warning);
            ...
            {
                EditorGUILayout.HelpBox(
                    "Found Photon PUN2, You can import integration package and open Photon demo scenes now.",
                    MessageType.Info);

                if (GUILayout.Button("Import Photon PUN2 Integration"))
                    AssetDatabase.ImportPackage(
                        RCCP_AddonPackages.Instance.GetAssetPath(RCCP_AddonPackages.Instance.PhotonPUN2), true);
            }
        }
        ...
        if (!mirrorAndRCCPInstalled)
        {
            ...
            {
                EditorGUILayout.HelpBox(
                    "If you want to add mirror demo scenes, you have to import latest Mirror to your project first.",
                    MessageType.Warning);
            ...
            {
                EditorGUILayout.HelpBox(
                    "Found Mirror, You can import integration package and open Mirror demo scenes now.",
                    MessageType.Info);

                if (GUILayout.Button("Import Mirror Integration"))
                    AssetDatabase.ImportPackage(
                        RCCP_AddonPackages.Instance.GetAssetPath(RCCP_AddonPackages.Instance.mirror), true);
            }
        }
    ...
    public static void Addons()
    {
        ...
        {
            AssetDatabase.ImportPackage(RCCP_GetAssetPath.GetAssetPath(RCCP_DemoContent.Instance.builtinShaderPackage),
                true);
            AssetDatabase.Refresh();
        }
        ...
        {
            AssetDatabase.ImportPackage(RCCP_GetAssetPath.GetAssetPath(RCCP_DemoContent.Instance.URPShaderPackage),
                true);
            AssetDatabase.Refresh();
        }
        ...
        if (GUILayout.Button("Remove Builtin Shaders"))
        {
            FileUtil.DeleteFileOrDirectory(
                RCCP_GetAssetPath.GetAssetPath(RCCP_DemoContent.Instance.builtinShadersContent));
            AssetDatabase.Refresh();
        }
        ...
        {
            EditorGUILayout.HelpBox("Installed Photon PUN2 with RCCP, You can open Photon demo scenes now.",
                MessageType.Info);
            EditorGUILayout.HelpBox(
                "If you want to remove Photon PUN2 integration from the project, delete the ''Photon PUN2'' folder inside the Addons/Installed folder. After that, you need to remove ''RCCP_PHOTON'' scripting define symbol in your player settings. In order to do that, go to Edit --> Project Settings --> Player Settings --> Other Settings, and remove the scripting symbol from the list.",
                MessageType.Warning, true);
        }
        ...
        {
            EditorGUILayout.HelpBox("Found BCG Shared Assets, You can open Enter / Exit demo scenes now.",
                MessageType.Info);
            EditorGUILayout.HelpBox(
                "If you want to remove BCG Shared Assets integration from the project, delete the ''BoneCracker Games Shared Assets'' folder. After that, you need to remove ''BCG_ENTEREXIT'' scripting define symbol in your player settings. In order to do that, go to Edit --> Project Settings --> Player Settings --> Other Settings, and remove the scripting symbol from the list.",
                MessageType.Warning, true);
        ...
        {
            EditorGUILayout.HelpBox("Installed Mirror with RCCP, You can open Mirror demo scenes now.",
                MessageType.Info);
            EditorGUILayout.HelpBox(
                "If you want to remove Mirror integration from the project, delete the ''Mirror'' folder inside the Addons/Installed folder. After that, you need to remove ''RCCP_MIRROR'' scripting define symbol in your player settings. In order to do that, go to Edit --> Project Settings --> Player Settings --> Other Settings, and remove the scripting symbol from the list.",
                MessageType.Warning, true);
        }
        ...
        if (GUILayout.Button("Import ProFlares Integration"))
            AssetDatabase.ImportPackage(RCCP_AddonPackages.Instance.GetAssetPath(RCCP_AddonPackages.Instance.ProFlare),
                true);
    ...
    private static void AddDemoScenesToBuildSettings()
    {
        ...
        EditorUtility.DisplayDialog("Demo Scenes",
            "All demo scenes have been added to the Build Settings. For Photon and Enter / Exit scenes, you have to import and intregrate them first (Addons).",
            "Ok");
    }
}