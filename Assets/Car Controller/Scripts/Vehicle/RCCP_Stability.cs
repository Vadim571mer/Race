...
public class RCCP_Stability : RCCP_Component
{
    ...
    private void Drift()
    {
        ...
        if (rearAxle)
        {
            rearWheelSlipAmountForward = (rearAxle.leftWheelCollider.wheelSlipAmountForward +
                                          rearAxle.rightWheelCollider.wheelSlipAmountForward) * .5f;
            rearWheelSlipAmountSideways = (rearAxle.leftWheelCollider.wheelSlipAmountSideways +
                                           rearAxle.rightWheelCollider.wheelSlipAmountSideways) * .5f;
        }

        float pRearWheelSlipAmountForward = (rearWheelSlipAmountForward * rearWheelSlipAmountForward) *
                                            Mathf.Sign(rearWheelSlipAmountForward);
        float pRearWheelSlipAmountSideways = (rearWheelSlipAmountSideways * rearWheelSlipAmountSideways) *
        ...
        CarController.Rigid.AddRelativeTorque(Vector3.up * CarController.steerInput_P * CarController.direction * 1f,
            ForceMode.Acceleration);

        CarController.Rigid.AddForceAtPosition(
            transform.forward * 3500f * Mathf.Abs(pRearWheelSlipAmountSideways) *
            Mathf.Clamp01(Mathf.Abs(pRearWheelSlipAmountForward * 10f)) * CarController.direction,
            comTransform.position, ForceMode.Force);
        CarController.Rigid.AddForceAtPosition(
            transform.right * 1000f * pRearWheelSlipAmountSideways *
            Mathf.Clamp01(Mathf.Abs(Mathf.Clamp(pRearWheelSlipAmountForward, .5f, 1f) * 10f)) * CarController.direction,
            comTransform.position, ForceMode.Force);
    }
    ...
    private void UpdateABS()
    {
        ...
        {
            if ((Mathf.Abs(brakedAxles[i].leftWheelCollider.wheelSlipAmountForward) * brakedAxles[i].brakeInput) >=
                engageABSThreshold)
            {
            ...
            if ((Mathf.Abs(brakedAxles[i].rightWheelCollider.wheelSlipAmountForward) * brakedAxles[i].brakeInput) >=
                engageABSThreshold)
            {
    ...
    private void UpdateESP()
    {
        ...
        if (underSteering)
        {
            ...
            {
                frontAxle.leftWheelCollider.AddBrakeTorque((frontAxle.maxBrakeTorque * (ESPIntensity * .1f)) *
                                                           Mathf.Clamp(-rearSlip, 0f, Mathf.Infinity));
                frontAxle.rightWheelCollider.AddBrakeTorque((frontAxle.maxBrakeTorque * (ESPIntensity * .1f)) *
                                                            Mathf.Clamp(rearSlip, 0f, Mathf.Infinity));
            }
        }
        ...
        if (overSteering)
        {
            if (rearAxle.isBrake)
            {
                rearAxle.leftWheelCollider.AddBrakeTorque((rearAxle.maxBrakeTorque * (ESPIntensity * .1f)) *
                                                          Mathf.Clamp(-frontSlip, 0f, Mathf.Infinity));
                rearAxle.rightWheelCollider.AddBrakeTorque((rearAxle.maxBrakeTorque * (ESPIntensity * .1f)) *
                                                           Mathf.Clamp(frontSlip, 0f, Mathf.Infinity));
            }
        }
    }
    ...
    private void SteerHelper()
    {
        ...
        CarController.Rigid.AddForce(-transform.right * localVelocity.x * steerHelperStrength * .015f,
        ...
        angularVelocity.y = Mathf.Lerp(angularVelocity.y,
            Mathf.Lerp(angularVelocity.y, adjustedSteerAngle, steerHelperStrength * .075f), 1f);
    ...
    {
        CarController.Rigid.angularDamping =
            Mathf.Lerp(0f, 10f, (Mathf.Abs(CarController.speed) * angularDragHelperStrength) / 1000f);
    }
    ...
    private void LimitDriftAngle()
    {
        ...
        if (Mathf.Abs(angle) > maxDriftAngle)
        {
            ...
            CarController.Rigid.angularVelocity = Vector3.Lerp(CarController.Rigid.angularVelocity, Vector3.zero,
                Time.fixedDeltaTime * driftAngleCorrectionFactor);
        }
    }
