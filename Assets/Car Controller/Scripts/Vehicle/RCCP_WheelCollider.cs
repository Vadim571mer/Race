...
public class RCCP_WheelCollider : RCCP_Component
{
    ...
    public override void Start()
    {
        ...
        skidAudioSource = RCCP_AudioSource.NewAudioSource(CarController.gameObject, "Skid Sound AudioSource", 5f, 50f,
            0f, null, true, true, false);
        ...
        newPivot.transform.SetPositionAndRotation(RCCP_GetBounds.GetBoundsCenter(wheelModel.transform),
            transform.rotation);
    ...
    private void WheelAlign()
    {
        ...
        wheelModel.transform.SetPositionAndRotation(wheelPosition,
            transform.rotation * Quaternion.Euler(this.wheelRotation, WheelCollider.steerAngle, 0f));
            ...
            for (int i = 0; i < RCCPGroundMaterials.terrainFrictions.Length; i++)
            {
                ...
                if (wheelHit.collider.sharedMaterial == RCCPGroundMaterials.terrainFrictions[i].groundMaterial)
                {
                    ...
                    for (int l = 0; l < RCCP_SceneManager.Instance.terrains.Length; l++)
                    {
                        ...
                        if (RCCP_SceneManager.Instance.terrains[l].terrainCollider ==
                            RCCPGroundMaterials.terrainFrictions[i].groundMaterial)
                        {
    ...
    private void SkidMarks()
    {
        ...
        if (totalSlip > RCCPGroundMaterials.frictions[groundIndex].slip)
        {
            ...
            if (CarController.Rigid.linearVelocity.magnitude > .1f && isGrounded && wheelHit.normal != Vector3.zero &&
                wheelHit.point != Vector3.zero && skidPoint != Vector3.zero && Mathf.Abs(skidPoint.magnitude) >= .1f)
                lastSkidmark = RCCP_SkidmarksManager.Instance.AddSkidMark(skidPoint, wheelHit.normal,
                    totalSlip - RCCPGroundMaterials.frictions[groundIndex].slip, width, lastSkidmark, groundIndex);
            else
                lastSkidmark = -1;
        }
        else
    ...
    private void MotorTorque()
    {
        ...
        if (cutTractionTCS != 0f && Mathf.Abs(wheelSlipAmountForward) > .05f)
        {
            ...
            {
                torque -= Mathf.Clamp(torque * (Mathf.Abs(wheelSlipAmountForward) * cutTractionTCS), 0f,
                    Mathf.Infinity);
                torque = Mathf.Clamp(torque, 0f, Mathf.Infinity);
            }
            else
    ...
    private void Frictions()
    {
        ...
        if (isGrounded)
        {
            ...
            totalSlip = Mathf.Lerp(totalSlip,
                ((Mathf.Abs(wheelHit.forwardSlip) + Mathf.Abs(wheelHit.sidewaysSlip)) / 2f), Time.deltaTime * 10f);
        }
        else
        {
        ...
        sidewaysFrictionCurve.stiffness =
            ((RCCPGroundMaterials.frictions[groundIndex].sidewaysStiffness * (1f - (handbrakeInput / 5f))) *
             connectedAxle.tractionHelpedSidewaysStiffness);
        handbrakeInput = 0f;
    ...
    private void Audio()
    {
        ...
        if (totalSlip > RCCPGroundMaterials.frictions[groundIndex].slip)
        {
            ...
            {
                skidAudioSource.volume = Mathf.Lerp(0f, skidVolume,
                    totalSlip - RCCPGroundMaterials.frictions[groundIndex].slip);
    ...
    public void SetFrictionCurvesForward(float extremumSlip, float extremumValue, float asymptoteSlip,
    ...
    /// <param name="asymptoteSlip"></param>
    /// <param name="asymptoteValue"></param>
    public void SetFrictionCurvesSideways(float extremumSlip, float extremumValue, float asymptoteSlip,
        float asymptoteValue)
    {
        WheelFrictionCurve newCurve = new WheelFrictionCurve();
        newCurve.extremumSlip = extremumSlip;
    ...
    public void ApplySteering(float steering)
    {
        ...
        if (steering > 0f)
        {
            if (transform.localPosition.x < 0)
                WheelCollider.steerAngle = (Mathf.Deg2Rad * steering * 2.55f) *
                                           (Mathf.Rad2Deg * Mathf.Atan(2.55f / (6 + (1.5f / 2))));
            else
                WheelCollider.steerAngle = (Mathf.Deg2Rad * steering * 2.55f) *
                                           (Mathf.Rad2Deg * Mathf.Atan(2.55f / (6 - (1.5f / 2))));
        }
    ...
    public void Deflate()
    {
        ...
        CarController.Rigid.AddForceAtPosition(transform.right * UnityEngine.Random.Range(-1f, 1f) * 25f,
    ...
    private void Drift()
    {
        ...
        {
            forwardFrictionCurve.extremumValue = Mathf.Clamp(forwardFrictionCurve_Def.extremumValue - (sqrVel / 1f),
                minForwardStiffnessForDrift, maxForwardStiffnessForDrift);
            forwardFrictionCurve.asymptoteValue = Mathf.Clamp(forwardFrictionCurve_Def.asymptoteValue + (sqrVel / 1f),
                minForwardStiffnessForDrift, maxForwardStiffnessForDrift);
        }
        else
        {
            forwardFrictionCurve.extremumValue = Mathf.Clamp(forwardFrictionCurve_Def.extremumValue - (sqrVel / .5f),
                minForwardStiffnessForDrift / 2f, maxForwardStiffnessForDrift);
            forwardFrictionCurve.asymptoteValue = Mathf.Clamp(forwardFrictionCurve_Def.asymptoteValue - (sqrVel / .5f),
                minForwardStiffnessForDrift / 2f, maxForwardStiffnessForDrift);
        }
        ...
        if (transform.localPosition.z < 0)
        {
            sidewaysFrictionCurve.extremumValue = Mathf.Clamp(sidewaysFrictionCurve_Def.extremumValue - (sqrVel / 1f),
                minSidewaysStiffnessForDrift, maxSidewaysStiffnessForDrift);
            sidewaysFrictionCurve.asymptoteValue = Mathf.Clamp(sidewaysFrictionCurve_Def.asymptoteValue - (sqrVel / 1f),
                minSidewaysStiffnessForDrift, maxSidewaysStiffnessForDrift);
        }
        else
        {
    ...
    public void AlignWheel()
    {
        ...
        transform.position += transform.up * (WheelCollider.suspensionDistance *
                                              (transform.root.localScale.y *
                                               (1f - WheelCollider.suspensionSpring.targetPosition)));
