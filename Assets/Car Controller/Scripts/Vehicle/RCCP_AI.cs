...
public class RCCP_AI : RCCP_Component
{
    ...
    private void Navigation()
    {
        ...
        float navigatorInput = Mathf.Clamp(transform.InverseTransformDirection(navigator.desiredVelocity).x * 1.25f,
            -1f, 1f);
        ...
        switch (navigationMode)
        {
            case NavigationMode.FollowWaypoints:
                ...
                {
                    throttleInput =
                        (distanceToNextWaypoint < (nextWaypointPassDistance * (Mathf.Abs(CarController.speed) / 30f)))
                            ? (Mathf.Clamp01(currentWaypoint.targetSpeed - Mathf.Abs(CarController.speed)))
                            : 1f;
                    ...
                    brakeInput =
                        (distanceToNextWaypoint < (nextWaypointPassDistance * (Mathf.Abs(CarController.speed) / 30f)))
                            ? (Mathf.Clamp01(Mathf.Abs(CarController.speed) - currentWaypoint.targetSpeed))
                            : 0f;
                    handbrakeInput = 0f;
            ...
            case NavigationMode.ChaseTarget:
                ...
                if (!reversingNow)
                {
                    ...
                    if (limitSpeed)
                        throttleInput *=
                            Mathf.Clamp01(Mathf.Lerp(10f, 0f, (Mathf.Abs(CarController.speed)) / maximumSpeed));
            ...
            case NavigationMode.FollowTarget:
                ...
                if (!reversingNow)
                {
                    throttleInput =
                        distanceToTarget <
                        (stopFollowDistance * Mathf.Lerp(1f, 5f, Mathf.Abs(CarController.speed / 50f)))
        ...
        if (!smoothedSteer)
            ...
            steerInput = Mathf.MoveTowards(steerInput, (ignoreWaypointNow ? rayInput : navigatorInput + rayInput),
                Time.deltaTime * 3f);
    ...
    private void FixedRaycasts()
    {
        ...
        for (int i = 0; i < anglesOfRaycasts.Length; i++)
        {
            ...
            Debug.DrawRay(pivotPos,
                Quaternion.AngleAxis(anglesOfRaycasts[i], transform.up) * transform.forward * raycastLength,
                Color.white);
            ...
            if (Physics.Raycast(pivotPos, Quaternion.AngleAxis(anglesOfRaycasts[i], transform.up) * transform.forward,
                    out hit, raycastLength, obstacleLayers) && !hit.collider.isTrigger &&
                ...
                switch (navigationMode)
                {
                    ...
                    case NavigationMode.ChaseTarget:
                        ...
                        if (targetChase && hit.transform != targetChase && !hit.transform.IsChildOf(targetChase))
                        {
                            //  Drawing hit gizmos.
                            Debug.DrawRay(pivotPos,
                                Quaternion.AngleAxis(anglesOfRaycasts[i], transform.up) * transform.forward *
                                raycastLength, Color.red);
                            casted = true;

                            //  Setting ray input related to distance to the obstacle.
                            if (i != 0)
                                rayInput -= Mathf.Lerp(Mathf.Sign(anglesOfRaycasts[i]), 0f,
                                    (hit.distance / raycastLength));
                        }
