...
public class RCCP_WheelBlur : RCCP_Component
{
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < wheelBlurRenderers.Length; i++)
        {
            ...
            if (wheelBlurRenderers[i] != null && wheelBlurRenderers[i].blurredWheel != null &&
                wheelBlurRenderers[i].wheelCollider != null)
            {
                //  Rotating the texture.
                wheelBlurRenderers[i].wheelRPM +=
                    (wheelBlurRenderers[i].wheelCollider.WheelCollider.rpm * (360f / 60f)) / 5f * Time.deltaTime;

                blurIntensity[i] = Mathf.Lerp(blurIntensity[i],
                    Mathf.Abs(wheelBlurRenderers[i].wheelCollider.WheelCollider.rpm / 2000f),
                ...
                wheelBlurRenderers[i].blurredWheel.transform.parent.position =
                    wheelBlurRenderers[i].wheelCollider.wheelModel.position;
                wheelBlurRenderers[i].blurredWheel.transform.parent.rotation = CarController.transform.rotation *
                                                                               Quaternion.Euler(
                                                                                   wheelBlurRenderers[i].wheelRPM *
                                                                                   rotationSpeed,
                                                                                   wheelBlurRenderers[i].wheelCollider
                ...
                if (wheelBlurRenderers[i].blurredWheel.transform.parent.localPosition.x > 0)
                {
                    ...
                    wheelBlurRenderers[i].blurredWheel.transform.parent.RotateAround(
                        wheelBlurRenderers[i].blurredWheel.transform.parent.position, CarController.transform.forward,
                        wheelBlurRenderers[i].wheelCollider.camber);
    ...
    public void Toggle()
    {
        ...
        for (int i = 0; i < wheelBlurRenderers.Length; i++)
        {
                ...
                wheelBlurRenderers[i].wheelCollider != null)
            {
                //  Assigning position and rotation of the texture.
                wheelBlurRenderers[i].blurredWheel.transform.parent.position =
                    RCCP_GetBounds.GetBoundsCenter(wheelBlurRenderers[i].wheelCollider.wheelModel);
                wheelBlurRenderers[i].blurredWheel.transform.parent.rotation = CarController.transform.rotation *
                                                                               Quaternion.Euler(
                                                                                   wheelBlurRenderers[i].wheelRPM,
                                                                                   wheelBlurRenderers[i].wheelCollider
