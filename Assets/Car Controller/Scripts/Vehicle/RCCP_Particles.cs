...
public class RCCP_Particles : RCCP_Component
{
    ...
    public override void Start()
    {
        ...
        if (wheelSparklePrefab && wheelSparkleList.Count < 1)
        {
            ...
            {
                GameObject sparks = Instantiate(wheelSparklePrefab,
                    CarController.AllWheelColliders[i].transform.position, Quaternion.identity);
        ...
        for (int i = 0; i < wheelParticles.Length; i++)
        {
            ...
            {
                GameObject ps = Instantiate(RCCPGroundMaterials.frictions[k].groundParticles, transform.position,
                    transform.rotation);
            ...
            for (int i = 0; i < CarController.AllWheelColliders.Length; i++)
            {
                if (CarController.AllWheelColliders[i].WheelCollider.enabled)
                {
                    if (CarController.AllWheelColliders[i].deflated &&
                        Mathf.Abs(CarController.AllWheelColliders[i].WheelCollider.rpm) >= 250f)
                    {
